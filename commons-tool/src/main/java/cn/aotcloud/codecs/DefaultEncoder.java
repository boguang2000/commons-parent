...
public class DefaultEncoder implements Encoder {
	...
	@SuppressWarnings("unchecked")
	public void setDefaultEncoder(List<String> codecNames) {
		for (String clazz : codecNames) {
			try {
				if (clazz.indexOf('.') == -1) {
					clazz = "cn.aotcloud.codecs." + clazz;
				}
	...
	@SuppressWarnings("unchecked")
	public DefaultEncoder(List<Object> codecClass) {
		for (Object clazz : codecClass) {
			codecs.add(clazz);
		}
	}
	...
	public String canonicalize(String input) {
		...
		// Issue 231 - These are reverse boolean logic in the Encoder interface, so we
		// need to invert these values - CS
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public String canonicalize(String input, boolean throwEnable, boolean logEnable) {
		if (input == null) {
			return null;
		}
		...
		if (foundCount >= 2 && mixedCount > 1) {
			if (throwEnable) {
				throw new SafeException("Input validation failure" + "Multiple (" + foundCount
						+ "x) and mixed encoding (" + mixedCount + "x) detected in " + input);
			}
			if (logEnable) {
				logger.warn(
						"Multiple (" + foundCount + "x) and mixed encoding (" + mixedCount + "x) detected in " + input);
			}
		} else if (foundCount >= 2) {
			if (throwEnable) {
				throw new SafeException(
						"Input validation failure" + "Multiple (" + foundCount + "x) encoding detected in " + input);
			}
			if (logEnable) {
			...
			if (throwEnable) {
				throw new SafeException(
						"Input validation failure" + "Mixed encoding (" + mixedCount + "x) detected in " + input);
			}
			if (logEnable) {
	 ...
	 * {@inheritDoc}
	 */
	public String encodeForURL(String input) throws SafeException {
		if (input == null) {
			return null;
		}
		try {
			return URLEncoder.encode(input, CHARACTER_ENCODING);
		} catch (UnsupportedEncodingException ex) {
			throw new SafeException("Encoding failure" + "Character encoding not supported", ex);
		} catch (Exception e) {
			throw new SafeException("Encoding failure" + "Problem URL encoding input", e);
		}
	}
	 ...
	 * {@inheritDoc}
	 */
	public String decodeFromURL(String input) throws SafeException {
		if (input == null) {
			return null;
		}
		String canonical = canonicalize(input);
		try {
			return URLDecoder.decode(canonical, CHARACTER_ENCODING);
		} catch (UnsupportedEncodingException ex) {
			throw new SafeException("Decoding failed" + "Character encoding not supported", ex);
		} catch (Exception e) {
			throw new SafeException("Decoding failed" + "Problem URL decoding input", e);
		}
	}
	 ...
	 * <a href="https://www.ietf.org/rfc/rfc3986.txt">RFC-3986</a> section 3, and it
	 ...
	 */
	public String getCanonicalizedURI(URI dirtyUri) throws SafeException {
...
//		         urn:example:animal:ferret:nose
		...
		for (UriSegment seg : set) {
			String value = canonicalize(parseMap.get(seg), true, true);
			value = value == null ? "" : value;
			// In the case of a uri query, we need to break up and canonicalize the internal
			// parts of the query.
			if (seg == UriSegment.QUERY && null != parseMap.get(seg)) {
				...
				try {
					...
					logger.debug("decoding error when parsing [" + dirtyUri.toString() + "]");
				}
			}
	...
	/**
	 * All the parts should be canonicalized by this point. This is straightforward
	 * assembly.
	 * 
	...
	protected String buildUrl(Map<UriSegment, String> parseMap) {
		StringBuilder sb = new StringBuilder();
		sb.append(parseMap.get(UriSegment.SCHEME)).append("://")
				// can't use SCHEMESPECIFICPART for this, because we need to canonicalize all
				// the parts of the query.
				// USERINFO is also deprecated. So we technically have more than we need.
				.append(parseMap.get(UriSegment.AUTHORITY) == null || parseMap.get(UriSegment.AUTHORITY).equals("") ? ""
						: parseMap.get(UriSegment.AUTHORITY))
				.append(parseMap.get(UriSegment.PATH) == null || parseMap.get(UriSegment.PATH).equals("") ? ""

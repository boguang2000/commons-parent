...
public class HTMLEntityCodec extends AbstractIntegerCodec {
	...
	/**
	 * Given an array of {@code char}, scan the input {@code String} and encode
	 * unsafe codePoints, except for codePoints passed into the {@code char} array.
	 * <br/>
	 * <br/>
	 * WARNING: This method will silently discard any code point per the call to
	 * {@code Character.isValidCodePoint( int )} method.
	 * 
	 * {@inheritDoc}
	 */
	@Override
	...
	public String encodeCharacter(char[] immune, int codePoint) {
		...
		// Cast the codePoint to a char because we want to limit immunity to the BMP
		// field only.
	 ...
	 * @return Returns the decoded version of the character starting at index, or
	...
	private Integer getNamedEntity(PushbackSequence<Integer> input) {
		...
		if (entry == null) {
			// We are lowercasing & comparing the result because of this all the upper case
			// named entities are getting converted lowercase named entities.
			// check is there any exact match
			// https://github.com/ESAPI/esapi-java-legacy/issues/302
			...
			if (!possibleString.equals(possibleStringLowerCase)) {
				Map.Entry<CharSequence, Integer> exactEntry = entityToCharacterTrie
						.getLongestMatch(possibleStringLowerCase);
				if (exactEntry != null)
					entry = exactEntry;
			}
			if (entry == null)

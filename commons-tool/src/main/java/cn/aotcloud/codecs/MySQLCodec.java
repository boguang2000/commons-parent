...
public class MySQLCodec extends AbstractCharacterCodec {
	/**
	 * Specifies the SQL Mode the target MySQL Server is running with. For details
	 * about MySQL Server Modes please see the Manual at
	 * 
	...
	public static enum Mode {
		...
		static Mode findByKey(int key) {
			...
			String message = String.format("No Mode for %s. Valid references are MySQLStandard: %s or ANSI: %s", key,
					MYSQL_MODE, ANSI_MODE);
	 ...
	 * 
	 * @param mode Mode has to be one of {MYSQL_MODE|ANSI_MODE} to allow correct
	 *             encoding
	 * @deprecated
	 ...
	 *
	 * Bug ###: In ANSI Mode Strings can also be passed in using the quotation. In
	 * ANSI_QUOTES mode a quotation is considered to be an identifier, thus cannot
	 * be used at all in a value and will be dropped completely.
	 * 
	...
	/**
	 * decodeCharacterMySQL decodes all the potential escaped characters that MySQL
	 * is prepared to escape
	 * 

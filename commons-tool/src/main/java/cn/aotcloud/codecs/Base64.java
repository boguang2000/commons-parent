...
public class Base64 {
	 ...
	 */
	public final static String ENABLE_UNSAFE_SERIALIZATION = "org.owasp.esapi.enableUnsafeSerialization"; // Do NOT
																											// change!
	...
	/**
	 * I don't get the point of this technique, but it is described here: <a href=
	 * "http://www.faqs.org/qa/rfcc-1940.html">http://www.faqs.org/qa/rfcc-1940.html</a>.
	 */
	...
	public static String encodeBytes(byte[] source, int off, int len, int options) {
		...
		if (gzip == GZIP) {
			...
			try {
				...
				logger.error("Problem writing gzip stream", e);
				return null;
			} // end catch
			finally {
				try {
	...
	private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
		...
		// Example: DkLE
			...
			try {
				...
				// logger.error( Logger.SECURITY_FAILURE, ""+source[srcOffset+1]+ ": " + (
				// DECODABET[ source[ srcOffset + 1 ] ] ) );
				// logger.error( Logger.SECURITY_FAILURE, ""+source[srcOffset+2]+ ": " + (
				// DECODABET[ source[ srcOffset + 2 ] ] ) );
				// logger.error( Logger.SECURITY_FAILURE, ""+source[srcOffset+3]+ ": " + (
				// DECODABET[ source[ srcOffset + 3 ] ] ) );

				// CHECKME: I replaced the 5 separate logger.error() calls above with a single
				// logger.error() call so they can't
				// become interleaved with other log entries from other threads. Normally this
				// would have placed log entries
				// on separate lines, so I also added line terminators here as well. (Probably
				// don't want it all on one single
				// really long log entry, do we?) Anyhow, somebody should check the formatting
				// to ensure that it's
				// esthetically pleasing, etc. But this works for me. I'm also OK if you want to
				// remove all the line terminators
	...
	public static byte[] decode(String s, int options) {
		...
		try {
			...
			// CHECKME: Is this correct? I think it should be a warning instead of an error
			// since nothing
			// is re-thrown. I do think that *some* sort of logging is in order here
			// especially since UTF-8 should
			// always be available on all platforms. If it's not, then all bets are off on
			// your runtime env. - Kevin Wall
			logger.warn("Problem decoding string using " + PREFERRED_ENCODING
					+ "; substituting native platform encoding instead", uee);
		}
	...
	public static boolean encodeFileToFile(String infile, String outfile) {
		...
		try {
			...
			logger.error("Problem encoding file to file", exc);
		} finally {
			try {
				in.close();
			} catch (Exception exc) {
	...
	public static boolean decodeFileToFile(String infile, String outfile) {
		...
		try {
			...
			logger.error("Problem decoding file to file", exc);
		} finally {
			try {
				in.close();
			} catch (Exception exc) {

...
package cn.aotcloud.oauth2.altu.oauth2.as.request;
...
public abstract class OAuthRequest {
    ...
    protected void validate() throws OAuthSystemException, OAuthProblemException {
        try {
            ...
            try {
                String redirectUri = HttpServletUtil.getParameter(request, OAuth.OAUTH_REDIRECT_URI);
    ...
    public String getParam(String name) {
        return HttpServletUtil.getParameter(request, name);
    }

    public String getClientId() {
        String[] creds = OAuthUtils.decodeClientAuthenticationHeader(HttpServletUtil.getHeader(request, OAuth.HeaderType.AUTHORIZATION));
        if (creds != null) {
            return creds[0];
        }
    ...
    public boolean isClientAuthHeaderUsed() {
        return OAuthUtils.decodeClientAuthenticationHeader(HttpServletUtil.getHeader(request, OAuth.HeaderType.AUTHORIZATION)) != null;
    }

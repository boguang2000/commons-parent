...
public class BearerBodyTokenExtractor implements TokenExtractor {
    ...
    public String getAccessToken(HttpServletRequest request) {
        String token = HttpServletUtil.getParameter(request, OAuth.OAUTH_BEARER_TOKEN);
        if (token == null) {
            token = HttpServletUtil.getParameter(request, OAuth.OAUTH_TOKEN);
        }
        return token;
    }
    ...
    public String getAccessToken(HttpServletRequest request, String tokenName) {
        return HttpServletUtil.getParameter(request, tokenName);
    }
}
